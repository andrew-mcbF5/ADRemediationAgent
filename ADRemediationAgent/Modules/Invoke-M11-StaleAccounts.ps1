...
function Invoke-M11 {
    ...
    try {
        ...
        Write-Host "  ✓ Quarantine OU exists" -ForegroundColor Green
    } catch {
    ...
    try {
        ...
        foreach ($u in $staleUsers) {
            Add-Finding -ObjectDN $u.DistinguishedName -FindingType "StaleUser" -Severity "MEDIUM" `
                -Description "User inactive since $($u.LastLogonDate?.ToString('yyyy-MM-dd') ?? 'Never logged in'). PwdLastSet: $($u.PasswordLastSet?.ToString('yyyy-MM-dd') ?? 'Unknown')"
        }
    } catch {
    ...
    try {
        ...
        foreach ($c in $staleComputers) {
            Add-Finding -ObjectDN $c.DistinguishedName -FindingType "StaleComputer" -Severity "LOW" `
                -Description "Computer inactive since $($c.LastLogonDate?.ToString('yyyy-MM-dd') ?? 'Never'). OS: $($c.OperatingSystem)"
        }
    } catch {
    ...
    if ($totalStale -eq 0) {
        Write-Host "  ✓ No stale accounts found above threshold" -ForegroundColor Green
    ...
    if (-not $quarantineOUExists) {
        ...
        try {
            ...
            if ($ouApproved) {
                ...
                Write-Host "  ✓ Quarantine OU created: $QuarantineOU" -ForegroundColor Green
        ...
        try {
            ...
            foreach ($u in $approvedUsers) {
                try {
                    ...
                    Write-Host "  ✓ Quarantined: $($u.SamAccountName)" -ForegroundColor Green
                    ...
                    Write-Host "  ✗ Failed for $($u.SamAccountName): $($_.Exception.Message)" -ForegroundColor Red
        ...
        try {
            ...
            foreach ($c in $approvedComputers) {
                try {
                    ...
                    Write-Host "  ✓ Quarantined: $($c.Name)" -ForegroundColor Green
                    ...
                    Write-Host "  ✗ Failed for $($c.Name): $($_.Exception.Message)" -ForegroundColor Red
